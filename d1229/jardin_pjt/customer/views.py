...
def clikes(request):
    if request.method=='POST':
        ...
        # db : board테이블의 likes컬럼에 좋아요 데이터 추가/삭제
        if board.likes.filter(pk=member.id).exists():# 다대다 검색
            # 제거
            board.likes.remove(member)# likes안의 member를 제거
            result='삭제'
        else:
    ...
    print('result :',result)
    context={'result':'성공','result':result,'count':count}
...
def cview(request,bno):
    ...
    print("다음글 : ",next_qs)
    context = {'c':qs,'prev_c':pre_qs,'next_c':next_qs,'comment_qs':comment_qs}
