...
# 게시판 목록
def list(request):
    qs=Board.objects.all().order_by('-bgroup')
...
def reply(request, bno):
    ...
    if request.method=='POST':
        ...
        id=request.session['session_id']
        qs2=Member.objects.get(id=id)
        # 1.bgroup의 부모보다 bstep이 더 높은 값을 검색
        bstepup_qs=Board.objects.filter(bgroup=bgroup,bstep__gt=bstep)
        # 검색된 데이터에서 bstep을 뽑아서 1씩 증가
        bstepup_qs.update(bstep=F('bstep')+1)
        # 답변달기 저장
        Board.objects.create(btitle=btitle,bcontent=bcontent,member=qs2,bgroup=bgroup,\
